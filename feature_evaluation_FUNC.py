...
def flistsforcat(whichclass):
    print("CLASS: "+str(whichclass))
    ...
    return F_list1,F_list2,F_list3,F_list4

F_list1 = np.zeros((8,1))
F_list2 = np.zeros((8,1))
F_list3 = np.zeros((8,1))
F_list4 = np.zeros((8,1))
for i in range(20):
    a,b,c,d = flistsforcat(i)
    F_list1+=np.array(a)
    F_list2+=np.array(b)
    F_list3+=np.array(c)
    F_list4+=np.array(d)
...
# %%
N_list = [10,20,50,100,200,500,1000,2000]
plt.plot(N_list,(F_list1-a)/17,label="BNS")
plt.plot(N_list,F_list2/17,label="|Accuracy|")
plt.plot(N_list,F_list3/17,label="PR")
plt.plot(N_list,F_list4/17,label="IG")
plt.xlabel("number of features selected")
plt.ylabel("F-measure")
plt.xscale('log')
plt.legend()
